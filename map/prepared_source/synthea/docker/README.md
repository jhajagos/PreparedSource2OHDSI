...
The mapping is done in a local [Spark](https://spark.apache.org/) environment which generates [Parquet files](https://parquet.apache.org/) that are 
...
such as, PostGreSQL. Microsoft SQL Server is widely used to host OHDSI CDM databases at different academic medical centers and is available 
...
A video overview of the mapping process can be viewed at [https://bit.ly/SyntheaOHDSI].

## Building the Synthea OHDSI Docker Image

For development purposes, I have Docker installed on my local machine. I have tested the mapping of 10,000 patients on a machine with 64Gb of RAM. 
...
The first step it is to build/compile the repository code:
```bash
cd ~
...
For testing purposes, I usually add the `--rm` option to remove the container after exiting which helps save space. The `-v` option specify the volume mount points from the external host file system.
...
In the container the CSV files are created in the directory `/root/synthea/output/csv`. An example of files that are generated:
```
...
The next step is to map the CSV files generated by the Synthea tool into OHDSI CDM compatible parquet files.
```bash
...
To emulate an environment found at a research institution we will load the data into a relational database. 
Here we will use the SQL Server Docker Image and run it locally.
...
Create a network and connect the two containers (execute this outside the container):
```bash
...
In this example we are exporting to Microsoft SQL Server. We need to link the scripts in the /root/scripts directory:
...
You will need to have created an empty database on the SQL Server database system and loaded the DDL 
...
```bash
cd /root/scripts
./load_all_staged_tables_into_ohdsi_rdbms.sh
```
Also, the script [transfer_sql_inserts_54.sql](..%2F..%2F..%2Fohdsi%2Futilities%2Fsqlserver%2Ftransfer_sql_inserts_54.sql) could also be run in your favorite SQL editor.
...
./fully_load_ohdsi_parquet_files_excluding_concepts_into_rdbms.sh # Load parquet files to a RDBMS system (Microsoft SQL Server)
```
